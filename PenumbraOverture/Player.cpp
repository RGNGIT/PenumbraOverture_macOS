...
cPlayer::cPlayer(cInit *apInit)  : iUpdateable("Player")
{
	...
	mvLastSafePosition = cVector3f(0,0,0);
...
{
	Log("cPlayer::Damage CALLED: afDamage=%f aType=%d health_before=%f\n", afDamage, (int)aType, mfHealth);
...
void cPlayer::Update(float afTimeStep)
{
	...
	//////////////////////////////////////////
	// Fell-through-the-level safety net.
	// A rare, not-yet-root-caused physics bug can occasionally (launch-
	// dependent, not consistently reproducible) let the character clip
	// through solid geometry when touching a dynamic object, dropping it
	// out of the level entirely. Track the last position where the
	// character was stably standing on the ground, and if it ever ends up
	// far below that, teleport it back rather than leaving the player
	// stuck falling through the world with no recovery.
	if(mpCharBody)
	{
		if(mpCharBody->IsOnGround())
		{
			mfTimeSinceSafePositionUpdate += afTimeStep;
			if(mfTimeSinceSafePositionUpdate > 0.3f)
			{
				mvLastSafePosition = mpCharBody->GetPosition();
				mbHasSafePosition = true;
			}
		}
		else
		{
		...
		if(mbHasSafePosition && mvLastSafePosition.y - mpCharBody->GetPosition().y > 5.0f)
		{
			mpCharBody->SetPosition(mvLastSafePosition);
