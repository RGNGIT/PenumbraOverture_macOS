...
{
	Log("OnHitGround: avVel.y=%f (thresholds: Min=%f Med=%f Max=%f) MoveState=%d\n",
		avVel.y, mfFallSpeed_Min, mfFallSpeed_Med, mfFallSpeed_Max, (int)mpPlayer->GetMoveState());
...
void cPlayerPickRayCallback::CalculateResults()
{
	...
	if(mpPickedAreaBody && mfPickedAreaDist < mfPickedDist)
	{
		...
		if(mpPickedBody)
		{
			...
			if(pEntity)
			{
				if(pEntity->GetName() == "cottonstrings")
				{
					Log("AREA-OVERRIDE CHECK 'cottonstrings': description='%s' mass=%f maxExamineDist=%f pickedDist=%f areaDist=%f\n",
						cString::To8Char(pEntity->GetDescription()).c_str(), mpPickedBody->GetMass(),
						pEntity->GetMaxExamineDist(), mfPickedDist, mfPickedAreaDist);
				}
