...
{
	Log("AddPickupCallback: item='%s' function='%s'\n", asItem.c_str(), asFunction.c_str());
...
void cInventory::AddUseCallback(const tString &asItem, const tString &asObject, const tString &asFunction)
{
	...
	for( ; it != m_mapUseCallbacks.end() && it->first == asItem; ++it)
	{
		cInventoryUseCallback *pCallback = it->second;
		if(pCallback && pCallback->msObject == asObject)
		{
			pCallback->msFunction = asFunction;
			return;
		}
...
void cInventory::CheckPickupCallback(const tString &asItem)
{
	...
	{
		Log("CheckPickupCallback: item='%s' -> NO callback registered\n", asItem.c_str());
		return;
	}
	cInventoryPickupCallback *pCallback = it->second;

	tString sCommand = pCallback->msFunction + "(\""+asItem+"\")";
	Log("CheckPickupCallback: item='%s' -> running '%s'\n", asItem.c_str(), sCommand.c_str());
...
bool cInventory::CheckUseCallback(const tString &asItem, const tString &asObject)
{
	...
	// Walk while the iterator is still valid and still pointing at an entry
	// for this item, rather than trusting count() to predict how many ++it
	// steps are safe. If the tree's internal structure is ever corrupted,
	// this stops at the first sign of trouble instead of walking off into
