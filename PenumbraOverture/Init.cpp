...
bool CheckSupport(cInit *apInit)
{
	...
	Log("Using Cg->GLSL translation backend (no separate profiles to check)\n");
...
bool cInit::Init(tString asCommandLine)
{
	...
	// If the user settings file doesn't exist yet, seed it from the
	// bundled defaults now, before the check below. Without this, every
	// single launch falls into the "use gsDefaultSettingsPath" branch
	// forever: cConfigFile::Save() always writes back to whatever path it
	// was constructed with, so settings changes end up saved inside the
	// Steam app's own install directory (config/default_settings.cfg)
	// rather than the user's per-account settings file - and Steam's
	// content integrity checks can silently revert modified files inside
	// the game's install directory on next launch, making changes look
	// like they never took effect even though the save briefly succeeded.
	if(!FileExists(gsUserSettingsPath))
	{
		FILE *pSrc = fopen(cString::To8Char(gsDefaultSettingsPath).c_str(), "rb");
		if(pSrc)
		{
			FILE *pDst = fopen(cString::To8Char(gsUserSettingsPath).c_str(), "wb");
			if(pDst)
			{
				char buf[4096];
				size_t lRead;
				while((lRead = fread(buf, 1, sizeof(buf), pSrc)) > 0)
				{
					fwrite(buf, 1, lRead, pDst);
				}
				fclose(pDst);
			}
			fclose(pSrc);
