		...
		while(entIt.HasNext())
		{
			...
			if(bRet)
			{
				...
				for(int i=0; i< collideData.mlNumOfPoints; i++)
				{
					...
					// Y is handled differently from X/Z on purpose: pushing
					// the character UP (out of the floor) is always safe,
					// but a downward push from one contact (e.g. a piece of
					// furniture with a bad/embedded collision normal) could
					// previously overwrite a correct, larger-magnitude
					// upward push from the actual floor contact - since this
					// loop iterates per body independently, taking the
					// largest abs() value per axis regardless of sign let a
					// bad downward value win and send the character through
					// the level. Only let a downward correction win if
					// there's no upward correction on the table at all.
					if(vPush.y >= 0)
					{
						if(vPushVec.y < vPush.y) vPushVec.y = vPush.y;
					}
					else if(vPushVec.y <= 0 && vPushVec.y > vPush.y)
					{
						vPushVec.y = vPush.y;
					}
				}
