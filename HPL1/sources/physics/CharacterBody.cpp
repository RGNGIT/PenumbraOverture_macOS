...
namespace hpl {
	...
	void iCharacterBody::Update(float afTimeStep)
	{
		...
		if(mvVelolcity.x !=0 || mvVelolcity.z!=0)
		{
			...
			{
				float fSafeTimeStep = afTimeStep > 0.0005f ? afTimeStep : 0.0005f;
				mvVelolcity.x = (vNewPos.x - vBeforeForcePos.x) * (1/fSafeTimeStep);
				mvVelolcity.z = (vNewPos.z - vBeforeForcePos.z) * (1/fSafeTimeStep);

				const float fMaxPlausibleSpeed = 20.0f;
				if(mvVelolcity.x > fMaxPlausibleSpeed) mvVelolcity.x = fMaxPlausibleSpeed;
				if(mvVelolcity.x < -fMaxPlausibleSpeed) mvVelolcity.x = -fMaxPlausibleSpeed;
				if(mvVelolcity.z > fMaxPlausibleSpeed) mvVelolcity.z = fMaxPlausibleSpeed;
				if(mvVelolcity.z < -fMaxPlausibleSpeed) mvVelolcity.z = -fMaxPlausibleSpeed;
			}
		...
		else
		{
			...
			// Guard against a division blow-up when afTimeStep is anomalously
			// tiny (e.g. an unstable/uncapped frame time without VSync). A
			// near-zero dt here would turn a trivial Y position nudge (like
			// bumping into a chair) into a nonsensical velocity spike, which
			// downstream gets misread as "fell from a great height" and
			// triggers fall damage / launches the character. 0.0005s (2000fps)
			// is far below any real frame time, so this never affects normal
			// gameplay physics.
			float fSafeTimeStep = afTimeStep > 0.0005f ? afTimeStep : 0.0005f;
			mvVelolcity.y = (vNewPos.y - vPosbeforeGrav.y) * (1/fSafeTimeStep);

			// Belt-and-suspenders: even with a sane dt, a single collision
			// resolution step can snap the character's position by a chunk
			// (pushing it out of an object it bumped into), which this
			// position-delta-over-dt formula would then misreport as an
			// extreme fall speed. Clamp to something well above any real
			// fall in this game but far below the spurious spikes seen from
			// simple furniture contact, so genuine hard falls still register
			// for fall damage while contact-driven snaps can't.
			const float fMaxPlausibleFallSpeed = 20.0f;
			if(mvVelolcity.y > fMaxPlausibleFallSpeed) mvVelolcity.y = fMaxPlausibleFallSpeed;
			if(mvVelolcity.y < -fMaxPlausibleFallSpeed) mvVelolcity.y = -fMaxPlausibleFallSpeed;
		...
		else
		{
			if(bCollide && vLastVelocity.y<=0 && std::abs(mvVelolcity.y) < (std::abs(vLastVelocity.y)-0.001f))
			{
				if(mbOnGround == false)
				{
					// Only treat this as a genuine fall-and-land event if the
					// character was actually airborne for a meaningful amount
					// of time. A single (or a handful of) frame(s) of bCollide
					// briefly failing near a jittering dynamic object (e.g.
					// furniture with residual physics noise) would otherwise
					// register as "in the air", letting gravity accumulate
					// velocity every such frame until it reaches terminal
					// speed over a few seconds of normal walking - then a real
					// ground contact fires fall damage for a fall that never
					// actually happened. 0.15s filters that out while still
					// catching real falls (which take far longer to develop
					// any meaningful velocity anyway).
					if(mfTimeInAir > 0.15f && mpCallback)
					{
		...
		{
			float fMoveDist = cMath::Vector3Dist(mvPosition, mvLastPosition);
			if(fMoveDist > 1.0f)
			{
				Log("CHARACTER POSITION JUMP: moved %f units in one step (from (%f,%f,%f) to (%f,%f,%f)) afTimeStep=%f mbOnGround=%d\n",
					fMoveDist, mvLastPosition.x, mvLastPosition.y, mvLastPosition.z,
					mvPosition.x, mvPosition.y, mvPosition.z, afTimeStep, (int)mbOnGround);
			}
		}
	}
