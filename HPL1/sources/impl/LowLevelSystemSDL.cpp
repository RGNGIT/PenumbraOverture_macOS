...
namespace hpl {
	...
	{
		int lRet = mpScriptEngine->RegisterGlobalFunction(asFuncDecl.c_str(),
												asFUNCTION(pFunc),callConv);
		if(lRet<0)
		{
			Error("Couldn't add func '%s' (AngelScript error code: %d)\n",asFuncDecl.c_str(),lRet);
			return false;
		}
