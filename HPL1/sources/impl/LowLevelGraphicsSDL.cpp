...
namespace hpl {
	...
	void cLowLevelGraphicsSDL::SetVsyncActive(bool abX)
	{
		...
		// wglSwapIntervalEXT/glXSwapIntervalSGI don't exist on macOS; this SDL 1.2
		// API (sdl12-compat on top of SDL3) controls swap interval via the
		// SDL_GL_SWAP_CONTROL GL attribute rather than a dedicated function.
		SDL_GL_SetAttribute(SDL_GL_SWAP_CONTROL, abX ? 1 : 0);
		#else
		SDL_GL_SetAttribute(SDL_GL_SWAP_CONTROL, abX ? 1 : 0);
		#endif
	}
	...
	{
		return hplNew( cCGProgram, (asName, aType) );
	}
