...
namespace hpl {
		...
		: iPhysicsWorld()
	{
		...
		// Force single-threaded contact/solver processing. Newton's default
		// multi-threaded broadphase (worker threads) can process contacts in
		// a different order depending on OS thread scheduling, which is
		// non-deterministic across separate process launches even with
		// identical code and level data. For borderline/near-stuck contact
		// configurations this can occasionally resolve badly (character
		// clipping through the level) on some launches but not others.
		// Single-threaded solving is slower but deterministic.
	...
	void cPhysicsWorldNewton::Simulate(float afTimeStep)
	{
...
#ifdef AS_ARM64_DEBUG
		{
			static bool bDumped = false;
			static std::map<cPhysicsBodyNewton*, cVector3f> mapStartPos;
			if(!bDumped && mlstBodies.size() > 10)
			{
				bDumped = true;
				Log("=== BODY NAME DUMP (%d bodies) ===\n", (int)mlstBodies.size());
				for(tPhysicsBodyListIt dbgIt = mlstBodies.begin(); dbgIt != mlstBodies.end(); ++dbgIt)
				{
					cVector3f vPos = (*dbgIt)->GetWorldPosition();
					cVector3f vSize = (*dbgIt)->GetBV()->GetSize();
					Log("  BODY: '%s' mass=%f pos=(%f,%f,%f) bvSize=(%f,%f,%f) collide=%d\n",
						(*dbgIt)->GetName().c_str(), (*dbgIt)->GetMass(), vPos.x, vPos.y, vPos.z,
						vSize.x, vSize.y, vSize.z, (int)(*dbgIt)->GetCollide());
					mapStartPos[static_cast<cPhysicsBodyNewton*>(*dbgIt)] = vPos;
				}
				Log("=== END BODY NAME DUMP ===\n");
			}

			static float fTrackTime = 0;
			fTrackTime += afTimeStep;
			static int lNextLogSecond = 1;
			if(fTrackTime > lNextLogSecond)
			{
				lNextLogSecond++;
				for(tPhysicsBodyListIt dbgIt = mlstBodies.begin(); dbgIt != mlstBodies.end(); ++dbgIt)
				{
					cPhysicsBodyNewton *pBody = static_cast<cPhysicsBodyNewton*>(*dbgIt);
					if(pBody->GetMass() <= 0) continue;

					std::map<cPhysicsBodyNewton*, cVector3f>::iterator itStart = mapStartPos.find(pBody);
					if(itStart == mapStartPos.end()) continue;

					cVector3f vPos = pBody->GetWorldPosition();
					float fDrop = itStart->second.y - vPos.y;
					if(fDrop > 3.0f || fDrop < -3.0f)
					{
						Log("BODY MOVED FAR t=%.1f: '%s' start=(%f,%f,%f) now=(%f,%f,%f) dropY=%f active=%d\n",
							fTrackTime, pBody->GetName().c_str(),
							itStart->second.x, itStart->second.y, itStart->second.z,
							vPos.x, vPos.y, vPos.z, fDrop, (int)pBody->IsActive());
					}
				}
			}
		}
...
#ifdef AS_ARM64_DEBUG
		std::vector<cVector3f> vPrePositions;
		vPrePositions.reserve(mlstBodies.size());
		for(tPhysicsBodyListIt dbgIt = mlstBodies.begin(); dbgIt != mlstBodies.end(); ++dbgIt)
			vPrePositions.push_back((*dbgIt)->GetWorldPosition());
#endif
		...
		{
			size_t dbgIdx = 0;
			for(tPhysicsBodyListIt dbgIt = mlstBodies.begin(); dbgIt != mlstBodies.end(); ++dbgIt, ++dbgIdx)
			{
				iPhysicsBody *pDbgBody = *dbgIt;
				if(pDbgBody->GetMass() <= 0) continue;
				cVector3f vDelta = pDbgBody->GetWorldPosition() - vPrePositions[dbgIdx];
				if(vDelta.Length() > 1.0f)
				{
					Log("TUNNELING SUSPECT: body='%s' mass=%f jumped %f units in one step (from (%f,%f,%f) to (%f,%f,%f))\n",
						pDbgBody->GetName().c_str(), pDbgBody->GetMass(), vDelta.Length(),
						vPrePositions[dbgIdx].x, vPrePositions[dbgIdx].y, vPrePositions[dbgIdx].z,
						pDbgBody->GetWorldPosition().x, pDbgBody->GetWorldPosition().y, pDbgBody->GetWorldPosition().z);
				}
			}
		}
#endif
...
#ifdef AS_ARM64_DEBUG
		if(pRigidBody->GetName() == "cottonstrings")
		{
			Log("RAYCAST HIT (prefilter): 'cottonstrings' active=%d\n", (int)pRigidBody->IsActive());
		}
#endif
								...
								int alCollisionID, void* apUserData, float afIntersetParam)
	{
...
#ifdef AS_ARM64_DEBUG
		bool bIsTarget = (pRigidBody->GetName() == "cottonstrings");
		if(bIsTarget)
		{
			Log("RAYCAST HIT (filter): 'cottonstrings' active=%d t=%f\n", (int)pRigidBody->IsActive(), afIntersetParam);
		}
#endif
		...
		if(bIsTarget)
		{
			Log("RAYCAST HIT (filter): 'cottonstrings' OnIntersect returned %d dist=%f\n", (int)bRet, gRayParams.mfDist);
		}
#endif
