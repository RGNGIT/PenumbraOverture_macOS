	...
	*/
		...
		cVector2f vTilePos,tVertexVec* apVtxVec, cColor ShadowColor,int lFirstIndex,float fSourceSize)
	{
		...
		for(int idx =0; idx<mlShadowPointSize; idx++)
		{
			...
			cVertex tempVtx0(vPointPos[0],ShadowColor); mpLowLevelGraphics->AddVertexToBatch(&tempVtx0);
			cVertex tempVtx1(vPointPos[1],ShadowColor); mpLowLevelGraphics->AddVertexToBatch(&tempVtx1);
			mpLowLevelGraphics->AddIndexToBatch(lFirstIndex);
			mpLowLevelGraphics->AddIndexToBatch(lFirstIndex+1);

			cVertex tempVtx2(vEndPos[0],ShadowColor); mpLowLevelGraphics->AddVertexToBatch(&tempVtx2);
			cVertex tempVtx3(vEndPos[1],ShadowColor); mpLowLevelGraphics->AddVertexToBatch(&tempVtx3);
			...
			if(bExtraPos){
				cVertex tempVtxExtra(vExtraPos,ShadowColor); mpLowLevelGraphics->AddVertexToBatch(&tempVtxExtra);
