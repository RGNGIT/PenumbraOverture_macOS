...
namespace hpl {
	...
	{
		// If the diffuse vertex program isn't available, RenderOcclusionQueries()
		// returns early without ever calling glBeginQuery/glEndQuery for any of
		// these query objects (see the matching check there). Waiting on their
		// results here would spin forever - the driver never marks an
		// unissued query as available. Skip fetching entirely in that case.
		if(mpDiffuseVtxProgram == NULL) return;
	...
	{
		// mpDiffuseVtxProgram can legitimately be NULL here - Cg shader
		// compilation isn't functional on this platform/port (see the
		// "Couldn't create program 'Diffuse_Color_vp.cg'" log line at
		// startup), and the constructor already tolerates that (checked at
		// creation, checked again before Destroy()). Occlusion queries are
		// a render-side optimization, not required for correctness, so
		// just skip them entirely rather than dereferencing a null program.
		if(mpDiffuseVtxProgram == NULL) return;
