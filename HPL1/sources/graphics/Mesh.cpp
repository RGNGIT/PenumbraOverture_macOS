												...
												const cMatrixf &a_mtxOffset,iPhysicsWorld *apPhysicsWorld)
	{
		...
		for(int joint=0; joint < GetPhysicsJointNum(); joint++)
		{
			...
			if(pParentBody ==NULL && pMeshJoint->msParentBody != ""){
				...
				// pChildBody was supposed to be constrained by this joint. With no
				// parent to attach it to, leaving it as a free dynamic body lets it
				// fall/swing unconstrained and can build up huge collision forces
				// (seen as e.g. a several-thousand-unit force spike on contact).
				// Make it static instead so a broken joint asset degrades to
				// "stuck in place" rather than "flying rigid body".
				if(pChildBody) pChildBody->SetMass(0);
				continue;
			}
			if(pChildBody ==NULL){
