...
namespace hpl {
	...
	private:
		...
		// NOTE: modern Newton passes a NewtonJoint* (a "contact joint",
		// potentially containing multiple individual contact points) here
		// instead of a single NewtonMaterial*/NewtonContact* pair. The old
		// separate EndContactCallback concept no longer exists in this
		// version's API - there is no third callback slot to register it in.
		static void ProcessContactCallback(const NewtonJoint* apContactJoint,
											dFloat afTimestep, int threadIndex);
