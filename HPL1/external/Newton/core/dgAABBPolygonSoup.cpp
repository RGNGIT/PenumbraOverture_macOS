...
*/
	...
	dgConstructionTree* ImproveTotalFitness (dgConstructionTree* const root, dgAABBTree* const boxArray, dgMemoryAllocator* const allocator)
	{
		...
		// If the tree is degenerate enough that root itself is the only
		// leaf (e.g. a collision mesh with just a handful of polygons),
		// PushNodes never appends anything to nodesList (it only collects
		// internal nodes, not leaves) and the list stays empty. GetLast()
		// on an empty list returns NULL, so just return the original root
		// unchanged in that case - there's nothing to rebalance anyway.
		if (nodesList.GetCount() == 0) {
			return root;
		}
