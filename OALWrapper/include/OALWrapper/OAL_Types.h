...
// Increased from the original 4096: at 44.1kHz 16-bit stereo, 4 buffers of
// 4096 bytes only cover ~93ms of audio - LESS than the default 100ms
// streaming-update polling interval (StreamUpdateFreq=10 in settings.cfg),
// leaving essentially no safety margin against normal OS scheduling jitter
// before a buffer runs dry (audible crackling). 16384 bytes/buffer gives
// ~372ms buffered with the same buffer count, comfortably covering the
// polling interval with headroom to spare.
#define STREAMING_BLOCK_SIZE 16384
